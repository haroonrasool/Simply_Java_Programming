...
public class SimpleEncryption extends JFrame {
    ...
    private void encryptJButtonActionPerformed(ActionEvent event) {
        // retrieving the numbers from numberJTextField.
        int number = Integer.parseInt(numberJTextField.getText());

        // extract individual digits
        int digit4 = number % 10; // pick off last digit
        number /= 10;             // eliminate last digit
        int digit3 = number % 10; // pick off last digit
        number /= 10;             // eliminate last digit
        int digit2 = number % 10; // pick off last digit
        number /= 10;             // eliminate last digit
        int digit1 = number;      // pick off remaining digit

        // encrypt each digit and display it
        encryptedDigit1JTextField.setText(
                String.valueOf((digit3 + 7) % 10));
        encryptedDigit2JTextField.setText(
                String.valueOf((digit4 + 7) % 10));
        encryptedDigit3JTextField.setText(
                String.valueOf((digit1 + 7) % 10));
        encryptedDigit4JTextField.setText(
                String.valueOf((digit2 + 7) % 10));
